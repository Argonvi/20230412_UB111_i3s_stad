...
```{r filter}
keep <- rowSums(assay(dds, "abundance") >= ntpm) >= min(table(dds$batch))
dds <- dds[keep,]
```
...
vsd <- vst(dds_adjusted_counts)
missing_genes <- immune_genes[!(immune_genes %in% rownames(vsd))]

library(org.Mm.eg.db)
ids <- AnnotationDbi::select(org.Mm.eg.db, missing_genes, "SYMBOL", "ALIAS")
  if(sum(is.na(entrez_id$ENTREZID > 0))){
    missing <- subset(entrez_id, is.na(ENTREZID))[,"SYMBOL"]
    found <- na.omit(AnnotationDbi::select(org.Mm.eg.db, missing, "ENTREZID", "ALIAS"))
  }
  entrez_id <- rbind(subset(entrez_id, !is.na(ENTREZID)), found)
  colnames(entrez_id) <- c("Gene","Entrez_ID")
  return(entrez_id)
}


vsd_immune <- vsd[immune_genes,]
```
